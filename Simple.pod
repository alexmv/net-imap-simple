...
=item put
  ...
  $imap->put( $mailbox_name, $message, $date, @flags ) or warn $imap->errstr;
...
A date may be supplied optionally per RFC 3501 - 6.3.11 APPEND
     "If a date-time is specified, the internal date SHOULD be set in
      the resulting message; otherwise, the internal date of the
      resulting message is set to the current date and time by default."
Note that there is not a specific format for the date string 
but found that "01-Jan-2016 12:59:00 -0500" works for Gmail IMAP
